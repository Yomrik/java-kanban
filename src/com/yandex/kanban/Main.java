...
//
        ...
        data.addTask(task4);
        Task task5 = new Task(StatusTask.NEW, "сделать обычную задачу №2", "обычная задача №2");
        data.addTask(task5);
        EpicTask epic6 = new EpicTask("Сделать БОЛЬШУЮ задачу №1", "БОЛЬШАЯ ЗАДАЧА №1");
        data.addEpic(epic6);
        Subtask subtask7 = new Subtask(StatusTask.NEW, "сделать маленькую задачу №1.1", "маленькая задача №1.1", epic6.getId());
        data.addSubtask(subtask7);
        Subtask subtask8 = new Subtask(StatusTask.NEW, "сделать маленькую задачу №1.2", "маленькая задача №1.2", epic6.getId());
        data.addSubtask(subtask8);
        EpicTask epic9 = new EpicTask("Сделать БОЛЬШУЮ задачу #2", "БОЛЬШАЯ ЗАДАЧА №2");
        data.addEpic(epic9);
        Subtask subtask0 = new Subtask(StatusTask.NEW, "сделать маленькую задачу №2.1", "маленькая задача №2.1", epic9.getId());
        data.addSubtask(subtask0);


        for (Task item : data.getTasks().values()) {
            System.out.println(item);
        }
        for (Task item : data.getEpics().values()) {
            System.out.println(item);
        }
        for (Task item : data.getSubtasks().values()) {
            System.out.println(item);
        }
        System.out.println("---------------");
